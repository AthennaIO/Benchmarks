...
export class Repl {
    ...
    static async boot() {
        ...
        Repl.log.write(`${Color.yellow.bold('To see all commands available type:')} .help\n`);
        ...
        repl.defineCommand('ls', {
            ...
            action() {
                ...
                if (nodeInternals.length) {
                    Repl.log.write(`${Color.green.bold('\nFROM NODE:')}\n\n${nodeInternals.join('\n')}`);
                }
                if (athennaInternals.length) {
                    Repl.log.write(`${Color.purple.bold('\nFROM ATHENNA:')}\n\n${athennaInternals.join('\n')}`);
                }
                Repl.log.write('');
                this.displayPrompt();
            },
     ...
     */
    static defineCleanCommand(repl) {
        repl.defineCommand('clean', {
            help: `Clean any property of REPL global context. Example: .clean ${Color.gray('(propertyName)')}`,
            action(property) {
                this.clearBufferedCommand();
                Repl.log.write('');
                if (!property) {
                    Repl.log.red('You have not provided any property to remove.');
                    Repl.log.write(`Try like this: .clean ${Color.gray('(propertyName)')}\n`);
                    return this.displayPrompt();
                }
                if (!repl.context[property]) {
                    Repl.log.red(`The property "${property}" doesn't exist inside REPL global context.`);
                    Repl.log.red('Use the ".ls" command to check the properties available in REPL global context.');
                    return this.displayPrompt();
                }
                delete repl.context[property];
                Repl.log.green(`Property "${property}" successfully removed from REPL global context.\n`);
                this.displayPrompt();
            },
        });
    }
