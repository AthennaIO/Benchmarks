...
export class Ignite {
    ...
    setApplicationSignals() {
        ...
        if (!signals.SIGTERM) {
            signals.SIGTERM = signal => {
                LoadHelper.shutdownProviders().then(() => process.kill(process.pid, signal));
            };
        }
    ...
    async setRcContentAndAppVars() {
        ...
        const __dirname = Module.createDirname(import.meta.url);
        const corePkgJson = await new File(resolve(__dirname, '..', '..', 'package.json')).getContentAsJson();
