...
export class Importer {
     ...
     */
    static async simpleImport(filePath) {
    ...
    static async import(filePath) {
        ...
        const { tests, afterAllHooks, afterEachHooks, beforeAllHooks, beforeEachHooks, } = Importer.getClassMetadata(Test);
        japaTest.group(Test.name, group => {
            beforeAllHooks.forEach(({ method }) => {
                const closure = bind(method);
                if (!closure)
                    return;
                group.setup(Importer.handleHook(method, Test.name, closure, BeforeAllHookException));
            });
            afterAllHooks.forEach(({ method }) => {
                const closure = bind(method);
                if (!closure)
                    return;
                group.teardown(Importer.handleHook(method, Test.name, closure, AfterAllHookException));
            });
            beforeEachHooks.forEach(({ method }) => {
                const closure = bind(method);
                if (!closure)
                    return;
                group.each.setup(Importer.handleHook(method, Test.name, closure, BeforeEachHookException));
            });
            afterEachHooks.forEach(({ method }) => {
                const closure = bind(method);
                if (!closure)
                    return;
                group.each.teardown(Importer.handleHook(method, Test.name, closure, AfterEachHookException));
            });
            Object.keys(tests.get()).forEach(method => TestConverter.convert(bind(method), tests.get(method)));
        });
    }
    /**
     * Creates a new closure to handle hook execution and errors.
     */
    static handleHook(method, className, closure, Exception) {
        return async () => {
            try {
                await closure();
            }
    ...
    static getClassMetadata(target) {
        return {
            tests: Reflect.getMetadata('tests', target) ||
