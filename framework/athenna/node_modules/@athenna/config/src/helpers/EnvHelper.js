...
export class EnvHelper {
     ...
     */
    static resolveFilePath(path = Path.pwd('.env'), override = this.isToOverrideEnvs()) {
    ...
    static getAppEnv(lookupNodeEnv) {
        ...
        if (this.isDefinedEnv(cliAppEnv)) {
            debug('application environment defined by --env flag with value %s.', cliAppEnv);
            return cliAppEnv;
        }
        const envName = process.env.APP_ENV ? 'APP_ENV' : 'NODE_ENV';
        const appEnv = process.env.APP_ENV || process.env.NODE_ENV;
        if (this.isDefinedEnv(appEnv)) {
            debug('application environment defined by %s env variable with value %s.', envName, appEnv);
            return appEnv;
        }
        if (!lookupNodeEnv) {
        ...
        if (!File.existsSync(Path.pwd('.env'))) {
            debug('unable to found env file at application root: %s. skipping %s lookup.', Path.pwd('.env'), envName);
            return null;
        }
        const content = new File(Path.pwd('.env')).getContentAsStringSync();
        if (!content) {
            debug('file %s content is empty, skipping %s lookup.', Path.pwd('.env'), envName);
            return null;
        }
    ...
    static getEnvFromString(envName, content) {
        ...
        if (this.isDefinedEnv(serializedValue) &&
            ...
            debug('found %s env variable with value %s inside file, returning it as environment value.', envName, serializedValue);
