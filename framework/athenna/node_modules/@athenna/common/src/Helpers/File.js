...
import { appendFileSync, createReadStream, createWriteStream, existsSync, mkdirSync, promises, readFileSync, rmSync, statSync, writeFileSync, ReadStream, WriteStream } from 'node:fs';
...
export class File {
    ...
    loadSync(options) {
        ...
        if (fileStat.size >= 2e8) {
            debug(`file %s with %s has been loaded in heap memory.`, this.base, this.fileSize);
        }
     ...
     */
    async getContentAsYaml(options) {
        this.loadSync({ isInternalLoad: true, withContent: false });
        const content = await this.getContentAsString(options);
