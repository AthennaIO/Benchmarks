...
export function Service(options) {
    return (target) => {
        ...
        debug('Registering %s dependency as a %s in the service container with %s alias.', target.name, options.type, alias);
        if (ioc.hasDependency(alias)) {
            debug('Dependency with alias %s already exists in the service provider, skipping registration of %s dependency.', alias, target.name);
            defineMetadata(target, options);
            return;
        }
        ioc[options.type](alias, target, createCamelAlias);
        debug('Dependency %s with alias %s registered in the service container as a %s dependency.', target.name, alias, options.type);
        defineMetadata(target, options);
    };
}
export function defineMetadata(target, options) {
