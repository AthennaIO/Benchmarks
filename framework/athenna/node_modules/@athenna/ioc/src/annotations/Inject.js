 ...
 */
export function Inject(alias) {
    return (target, key) => {
        if (!alias) {
            alias = key;
        }
        debug('Injecting %s dependency alias in %s dependency.', alias, target.constructor.name);
        ...
        debug('Dependency alias %s injected in %s dependency.', alias, target.constructor.name);
