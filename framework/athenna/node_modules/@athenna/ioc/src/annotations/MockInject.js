 ...
 */
export function MockInject(dependency) {
    return (target, key) => {
        ...
        debug('Registering %s dependency as singleton in the service container with %s alias.', dependency.name, alias);
        ioc.singleton(alias, dependency, true);
        Object.defineProperty(target, key, {
            value: sinon.mock(ioc.safeUse(alias)),
        });
        debug('Dependency %s registered as a mock in %s target.', dependency.name, target.constructor.name);
    };
}
