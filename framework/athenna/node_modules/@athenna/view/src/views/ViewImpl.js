...
export class ViewImpl {
     ...
     * ```
     */
    async renderRawByPath(path, data) {
    ...
    createViewDisk(name, path) {
        if (!path) {
            ...
            if (!isAbsolute(name)) {
                debug('Path %s is not absolute and is going to be resolved using cwd %s.', name, Path.pwd());
        ...
        if (!isAbsolute(path)) {
            debug('Path %s for view disk %s is not absolute and is going to be resolved using cwd %s.', path, name, Path.pwd());
     ...
     * ```
     */
    createComponentByPath(name, path) {
        if (!isAbsolute(path)) {
            debug('Path %s for view disk is not absolute and is going to be resolved using cwd %s.', path, Path.pwd());
            path = resolve(Path.pwd(), path);
        }
     ...
     * ```
     */
    createTemplateByPath(name, path) {
        if (!isAbsolute(path)) {
            debug('Path %s for view disk is not absolute and is going to be resolved using cwd %s.', path, Path.pwd());
            path = resolve(Path.pwd(), path);
        }
    ...
    removeTemplate(name) {
        if (!this.hasTemplate(name)) {
            debug('Template %s does not exist and removing operation will be skipped.', name);
            return this;
        }
