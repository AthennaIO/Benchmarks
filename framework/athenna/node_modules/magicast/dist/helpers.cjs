...
const builders = require('./shared/magicast.a2f6285b.cjs');
require('recast');
require('node:fs');
...
function getConfigFromVariableDeclaration(magicast) {
  if (magicast.exports.default.$type !== "identifier") {
    throw new builders.MagicastError(
    ...
    if (node.type === "VariableDeclaration") {
      for (const declaration of node.declarations) {
        if (declaration.id.type === "Identifier" && declaration.id.name === configDecalarationId && declaration.init) {
          ...
          const configExpression = builders.parseExpression(builders.generateCode(init).code);
          return {
            declaration,
  ...
  throw new builders.MagicastError("Couldn't find config declaration");
}
...
function insertPluginIntoVariableDeclarationConfig(magicast, plugin) {
  ...
  if (declaration.init) {
    if (declaration.init.type === "ObjectExpression") {
      declaration.init = builders.generateCode(configObject).code;
    } else if (declaration.init.type === "CallExpression" && declaration.init.callee.type === "Identifier") {
      declaration.init = builders.generateCode(
        builders.builders.functionCall(declaration.init.callee.name, configObject)
      ).code;
    }
  }
}
